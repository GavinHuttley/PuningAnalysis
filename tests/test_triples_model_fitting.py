import pytest
from clock_project.genome_analysis.triples_model_fitting import (
    check_process,
    triples_model_fitting,
)
...
def test_check_process(aln_path, result_lf_dir, triples_info_dir):
    triples_speccies_infos, result, triples_aln = check_process(
        aln_path, result_lf_dir, triples_info_dir
    )
    ...
    assert set(triples_speccies_infos[identifier_number].values()) == set(
        triples_aln.names
    )
    assert result.source.split(".")[0] == "ENSG00000009307"


def test_triples_model_fitting(
    aln_path, result_lf_dir, triples_info_dir, triples_species_names
):
    triples_species_infos, result, triples_aln = check_process(
        aln_path, result_lf_dir, triples_info_dir
    )
    triples_alignment_paths = open_data_store(aln_path, mode="r", suffix="json")
    identifier_number = triples_alignment_paths[0].unique_id.split(".")[0]
    ens_tree = result.lf.get_ens_tree()
    model_fitting_result = triples_model_fitting(
        triples_aln, triples_species_names, ens_tree
    )
    ...
    assert (
        model_fitting_result.tree.get_newick()
        == ens_tree.get_sub_tree(triples_species_names.values()).get_newick()
    )
