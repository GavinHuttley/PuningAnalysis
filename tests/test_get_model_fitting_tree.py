...
import pytest
from clock_project.genome_analysis.get_model_fitting_tree import (
    decorate_vertebrate_tree,
    model_fitting_tree,
)
...
def common_names_mapping():
    with open(
        "/Users/gulugulu/repos/PuningAnalysis/results/output_data/common_names_mapping.json",
        "r",
    ) as common_name_infile:
...
def test_decorate_vertebrate_tree(
    vertabrate_tree_path, common_names_mapping, tip_names
):
    tree = decorate_vertebrate_tree(vertabrate_tree_path, common_names_mapping)
...
def test_model_fitting_tree(aln_path, sub_tree):
    ...
    assert set(model_fitting_result.lf.tree.get_tip_names()) == set(
        sub_tree.get_tip_names()
    )
