import pytest
from clock_project.genome_analysis.triple_selection import (
    get_ingroup_pair,
...
def triples_species_name():
    return {
        "ingroup1": "Arctic_ground_squirrel",
...
def triples_info_big_tree():
    return {
        "triples_info_big_tree": {
            "internal_node": "Rodentia.3",
            "root": "Glires",
            "jsd_dict": {
                "Ingroup_JSD": 0.0016731612268652274,
                "JSD_difference": {
                    "Arctic_ground_squirrel": 0.002180861209883478,
...
def test_get_ingroup_pair(sequences):
    ...
    assert set(ingroup_pair) <= set(all_speices), (
        "ingroup pair is not species in the sequence."
    )
...
def test_get_outgroup_species(triples_species_name, ens_tree, expected_relationship):
    ...
    assert set(outgroup) <= set(expected_relationship["cousion"]), (
        "Outgroup species is not the sibling of lca."
    )
...
def test_get_triples_info(
    triples_species_name,
    ens_tree,
    triples_info_big_tree,
    lca_parent_node,
    sequences,
    ...
    assert (
        traids_info[0]["triples_info_big_tree"]
        == triples_info_big_tree["triples_info_big_tree"]
    ), "there is difference in triples_info."
