...
from clock_project.data_processing.data_filter_seqs import (
    common_name_renamer,
    drop_ambiguous,
    drop_low_matching,
    drop_short_seq,
    get_matching_score_to_human,
    get_score_at_quantile,
...
@pytest.fixture
def sub_seqs(seqs):
    return seqs.take_seqs(
        [
            "echinops_telfairi-ENSETEG00000000236",
            "dasypus_novemcinctus-ENSDNOG00000018355",
            "loxodonta_africana-ENSLAFG00000013661",
        ]
    )
...
def test_drop_ambiguous(seqs, quantile2):
    ...
    for name in seqs_filtered_lengths_wa.keys():
        assert (
            seqs_filtered_lengths_woa[name] / seqs_filtered_lengths_wa[name]
            >= 1 - quantile2
        )
