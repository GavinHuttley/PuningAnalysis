...
def test_number_of_simulations(setup_simulation, max_time):
    ...
    assert len(results) == expected_num_simulations, (
        "The number of simulations does not match the expected."
    )
...
def test_sequence_length(setup_simulation, max_time):
    ...
    all_lengths_correct = all(
        len(seq) == expected_length for result in results for seq in result
    )
...
def test_deterministic_output(setup_simulation, Q_matrix, max_time):
    ...
    other_simulator = SeqSimulate(
        Q=Q_matrix, length=100, num_repeat=5, seed=123, pi=[0.15, 0.25, 0.35, 0.25]
    )
    other_results = other_simulator.main(max_time)
    results = setup_simulation.main(max_time)
    assert all(results) == all(other_results), (
        "The output sequence does not match the expected deterministic sequence."
    )
    ...
    for simulation in results:
        for i in range(1, len(simulation)):
            ...
            assert differences == 1, (
                f"Found {differences} differences between sequences, expected exactly 1."
            )


@pytest.mark.parametrize("expected_count", [2500])
def test_generate_ancestor_nucleotide_distribution(
    equal_frequency, sequence_length, expected_count
):
