...
def get_rehected_proportion(p_value_observed_dict):
    ...
    proportion_less_than_005_toc = {gene: rejected_cases_count[gene] / total for gene, total in total_count.items()}
...
def get_rejected_proportion_plot(rejected_proportion_tos, rejected_proportion_toc):
    ...
    fig = px.histogram(
        ...
        labels={'x': 'Rejected proportion', 'y': 'Count'},  # Label for y-axis as 'Count'
        title=None,
...
def get_triple_ens_diff_distirbution_plot(path):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=4, color='#67a8cd'), name='Ingroup JSD'
        ),
        row=2, col=1
    )
