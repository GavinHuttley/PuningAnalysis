...
def get_proportion_rejected_correlation_fig(
    proportion_less_than_005_tos, proportion_less_than_005_toc
):
    proportion_less_than_005_clock_filtered = {
        gene: proportion_less_than_005_toc[gene]
        for gene in proportion_less_than_005_tos.keys()
    }
    ...
    fig = px.scatter(
        ...
        y=y_values,
        labels={
            "x": "Proportion reject the clock",
            "y": "Proportion reject the stationarity",
        },
        trendline="ols",
        title=None,
    )
    ...
    corr, p = spearmanr(
        list(proportion_less_than_005_tos.values()),
        list(proportion_less_than_005_toc.values()),
    )
    # fig.add_annotation(
    #     x=1, y=1,
    ...
    for path in input_data_dir:
        ...
        if not isinstance(bootstrap_result, NotCompleted):
            ...
            while pvalue is None:
                null_sample_key = sample(
                    list(bootstrap_result.keys())[1:], 1
                )  # Sampling from keys, skip first key
...
def get_rejected_proportion(p_value_observed_dict):
    ...
    proportion_rejected = {
        gene: count_less_than_005_clock[gene] / total
