...
def compute_aginst_Q(matrices_list, pi, t_range):
    ...
    df = pd.DataFrame(
        result_list,
        columns=["Matrix_ID", "Time", "ENS_abs_difference", "JSD_difference"],
    )
    return df
...
def correlation_factor_plot(df_low, df_high, t_range):
    ...
    correlation_factors_low_jad = {
        time: correlation_results_low_jad[time][0]
        for time in correlation_results_low_jad
    }
    correlation_factors_high_jad = {
        time: correlation_results_high_jad[time][0]
        for time in correlation_results_high_jad
    }
