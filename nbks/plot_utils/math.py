...
from clock_project.maths.evolutionary_rate import calculate_stationary_distribution, calculate_stationary_rate, calculate_non_stationary_rate, calculate_non_stationary_mu_prime
...
def generate_ENS(Q, t_range, pi=None):
    ...
    for t in t_range:
        ens = expected_number_subs(pi, Q, t) # Update the accumulated ENS at t1 to continue from this point using Q2
...
def get_ens_plot(Q, t_range, pi=None):
    ...
    fig.add_trace(go.Scatter(x=t_range, y=ens_values, mode='lines', line = dict(color = '#cd6d2e', width = 4)))
...
def get_evolutionary_rate_change_plot(Q1, Q2, Q3, pi, t_range):
    ...
    for i, Q in enumerate(Q, start=1):
        ...
        fig.add_trace(
            go.Scatter(x=x_plot, y=y, mode='lines',
                       line=dict(color='#58B8D1', width=4), showlegend=False),
            row=1, col=i
        )
...
def get_clock_violation_change_two_process(Q1, Q2, t_range, pi = None):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=t_range, y=mu_range1, mode='lines', name='Species 2', line=dict(color='#AC8AB3', width=3)))

    fig.add_trace(go.Scatter(x=t_range, y=mu_range2, mode='lines', name='Species 1', line=dict(color='#62BE9F', width=3)))

    fig.add_trace(go.Scatter(x=t_range, y=np.abs(np.array(mu_range1) - np.array(mu_range2)), mode='lines', name='Difference (Clock violation)', line=dict(color='#ff0b00', width=3, dash='dash')))
