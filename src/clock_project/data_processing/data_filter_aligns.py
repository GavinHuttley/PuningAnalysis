...
@define_app
def replace_common_species_names(alignment: SeqsCollectionType) -> SeqsCollectionType:
    with open(
        "/Users/gulugulu/clock/mammal_orthologs_hsap_1/available_species_name.json", "r"
    ) as infile1:
        available_species_names = json.load(infile1)

    with open(
        "/Users/gulugulu/clock/mammal_orthologs_hsap_1/common_names.json", "r"
    ) as infile2:
...
@define_app
def filter_by_num_seqs(
    data: SeqsCollectionType, min_num_seqs: int
) -> SeqsCollectionType:
    """
