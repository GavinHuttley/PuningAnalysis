...
def initialize_waiting_times_vectorized(DNA_seq, Q_dict):
    ...
    for next_base in range(4):
        rates = np.array(
            [
                Q_dict[curr_base, next_base] if curr_base != next_base else float("inf")
        ...
        waiting_times[valid_rates, next_base] = np.random.exponential(
...
def simulate_seq(max_time, rate_matrix, ancestor_seq=None):
    ...
    while time_passed + min_time <= max_time:
        ...
        seq_index, new_base = min_position
        min_position, min_time = update_waiting_times(
            DNA_seq, rate_matrix, min_position, min_time
        )
