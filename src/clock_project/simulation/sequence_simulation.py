...
from clock_project.simulation.wts import taxonomic_triple_simulation
...
seed_ancestor_seqs = {
    key: [int(str) for str in seed_ancestor_seqs_list[key]]
    for key in seed_ancestor_seqs_list.keys()
}


def simulate_taxonomic_triples(
    ancestor_seq, Q_group, pi, seed, t, path_to_dir, ens_dict, Q_collection
):
...
@click.option(
    ...
    "-t",
    default="[0, 1]",
    callback=lambda _, __, x: ast.literal_eval(x),
...
@click.option(
    "--pi_list",
    "-pi",
    default="[[0.25, 0.25, 0.25, 0.25], [0.49, 0.02, 0.35, 0.14]]",
    callback=lambda _, __, x: ast.literal_eval(x),
...
@click.option(
    ...
    "-md",
    type=click.Path(),
    help="matrix_collection directory",
)
...
def main(
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        pool.starmap(
            ...
            [
                (combo, time_list, matrix_collection, seed, output_dir)
