...
def generate_rate_matrices(markov_order: int):
    ...
    if markov_order == 0:
        ...
        rate_matrices["0"] = (
            rate_matrix  # For standardizing so that output is always a dictionary of matrices.
        )
        ...
        nucleotides = ["0", "1", "2", "3"]
        one_side_neighbours = [
            "".join(p) for p in itertools.product(nucleotides, repeat=markov_order)
        ]
...
def get_context(seq_index, DNA_seq, markov_order, anchor_base="0"):
    ...
    if markov_order == 0:
        ...
        if (seq_index < markov_order) & (
            len(DNA_seq) - (seq_index + 1) < markov_order
        ):  # handles incomplete left and right neighbours
            left_base = "".join(
                [(anchor_base * (markov_order - seq_index)), DNA_seq[:seq_index]]
            )
            right_base = "".join(
                [
                    DNA_seq[seq_index + 1 :],
                    (anchor_base * (markov_order - (len(DNA_seq) - seq_index - 1))),
                ]
            )
            ...
            len(DNA_seq) - (seq_index + 1) < markov_order
        ):  # handles incomplete right neighbour
...
def get_neighbour_indices(DNA_seq, seq_index, markov_order):
    ...
    if (seq_index < 0) | (seq_index >= sequence_length):
        ...
        for index in range(
            max(0, seq_index - markov_order),
            min(sequence_length, seq_index + markov_order + 1),
        ):
...
def update_waiting_times(
    DNA_seq, Q_dict, waiting_times, min_position, min_time, markov_order
):
    """
    Input:
        DNA_seq (list): The DNA sequence as a list.
    ...
    seq_index = min_position[
        0
    ]  # obtain the index of the current base that is about to be substituted in the sequence

    new_base = min_position[
        1
    ]  # obtain the new base that is going to substitute the current base

    DNA_seq[seq_index] = f"{new_base}"  # Substitue with new base in DNA sequence

    # Regenerate all waiting times
    waiting_times, min_position, min_time = initialize_waiting_times(
        DNA_seq, Q_dict, markov_order
    )
...
def simulate_seq(ancestor_seq, max_time, rate_matrices_dict, markov_order):
    ...
    while time_passed <= max_time:
        ...
        waiting_times, min_position, min_time = update_waiting_times(
            DNA_seq, Q_dict, waiting_times, min_position, min_time, markov_order
        )
        DNA_seq[seq_index] = f"{new_base}"  # Substitue with new base in DNA sequence
    ...
    if (
        len(history) <= 2
    ):  # meaning time for any substitution to occur exceeded the max_time, no substitution took place
