...
def get_triples_info(model_fitting_result, triples_species_name):
    ...
    ingroup_species_gene_name = [
        triples_species_name["ingroup1"],
        triples_species_name["ingroup2"],
    ]
    outgroup = triples_species_name["outgroup"]
    matrices = {
        n: model_fitting_result.lf.get_rate_matrix_for_edge(n, calibrated=True)
        for n in triples_species_name.values()
    }
    sub_ens_tree = model_fitting_result.lf.get_ens_tree()
    lca_node = sub_ens_tree.lowest_common_ancestor(
        [triples_species_name["ingroup1"], triples_species_name["ingroup2"]]
    ).get_node_names()[0]
    ...
    distribution_internal_root = list(
        model_fitting_result.lf.get_motif_probs_by_node()[lca_node]
    )
    ...
    ens_dict = {
        n: sub_ens_tree.to_rich_dict()["edge_attributes"][n]["length"]
        for n in triples_species_name_list
    }
    ens_difference = abs(
        ens_dict[ingroup_species_gene_name[0]] - ens_dict[ingroup_species_gene_name[1]]
    )
    nabla_dict = {}
    for species in ingroup_species_gene_name:
        nabla_dict[species] = calculate_non_stationarity(
            distribution_internal_root, matrices[species], ens_dict[species]
        )
    nabla_dict[outgroup] = calculate_non_stationarity(
        distribution_root, matrices[outgroup], ens_dict[outgroup]
    )
    shortest_ens = min(ens_dict.values())
    matrices_list = {
        n: matrices[n].to_array().tolist() for n in triples_species_name.values()
    }
    distributions = {
        "internal_node": distribution_internal_root,
        "root": distribution_root,
        "ingroup1": list(nuc_freqs1_reordered),
        "ingroup2": list(nuc_freqs2_reordered),
    }
    triad_data = {
        "triples_species_names": triples_species_name,
        "triples_info_small_tree": {
...
def process_path(path, triples_info_dir):
    ...
    print(file_name)
    triples_info_path = os.path.join(
        triples_info_dir, file_name, "triples_species_names_dict.json"
    )
    triples_info = deserialise_object(triples_info_path)
    model_fitting_result_path = os.path.join(path, "model_fitting_result")
    model_fitting_result_input_dstore = open_data_store(
        model_fitting_result_path, suffix="json"
    )
...
@click.option(
    "--triples_info_dir",
    "-td",
    help="Path to the directory containing info of all triples",
)
@click.option(
    "--num_processes",
    "-n",
