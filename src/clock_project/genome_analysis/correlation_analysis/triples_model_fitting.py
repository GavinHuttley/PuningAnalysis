...
def triples_model_fitting(triples_aln, triples_species_name, ens_tree):
    ...
    model_triples = get_app(
        "model",
        "GN",
        tree=tree_topology_triples,
        time_het="max",
...
def process_path(path, result_lf_dir, triples_info_dir, output_dir):
    ...
    for alignment_path in triples_alignment_paths:
        ...
        model_fitting_result = triples_model_fitting(
            triples_aln, triples_species_name, ens_tree
        )
...
@click.option(
    "--triples_info_dir",
    "-td",
    help="Path to the directory containing model fitting result",
)
@click.option(
    "--result_lf_dir",
    "-rd",
...
@click.option(
    ...
    "-o",
    type=click.Path(),
    help="Output directory for model fitting result of taxanomic triples",
)
@click.option("--limit", "-l", type=int, help="Limit the number of files to process")
def main(
    triples_alignment_dir,
    triples_info_dir,
    result_lf_dir,
    num_processes,
    output_dir,
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        pool.starmap(
            process_path,
            [(path, result_lf_dir, triples_info_dir, output_dir) for path in paths],
        )
