...
def get_trace_back_nodes(
    ingroup_species_gene_name, outgroup, lca_node, parent_node, ens_tree
):
    trace_back_nodes = {}
...
def get_triples_info(
    ...
    sub_tree = ens_tree.get_sub_tree(triples_species_names.values())
    ingroup_species_gene_name = [
        triples_species_names["ingroup1"],
        triples_species_names["ingroup2"],
    ]
    ...
    ens_difference = np.sqrt(
        (
            ens_dict[ingroup_species_gene_name[0]]
            - ens_dict[ingroup_species_gene_name[1]]
        )
        ** 2
    )
    ...
    jsd_dict = {
        ...
        "JSD_difference": {
            ingroup_species_gene_name[0]: jsd1,
            ingroup_species_gene_name[1]: jsd2,
        },
    }
    nuc_freqs_dict = {
        "Ingroup_nuc_freqs": {
            ingroup_species_gene_name[0]: list(nuc_freqs1),
            ingroup_species_gene_name[1]: list(nuc_freqs2),
        },
    ...
    triple_data = {
        ...
        "triples_species_names": triples_species_names,
        "triples_info_big_tree": {
...
def triples_binning_ens_diff(triple_data, ens_diff_bins):
    ...
    if (
        ens_diff_bin_index in ens_diff_bins["bins"]
        and ens_diff_bins["bins"][ens_diff_bin_index] is None
    ):
...
def process_path(path, output_dir, sequence_dir, alignment_dir, output_alignment_dir):
    ...
    jsd_bins = {
        "size": jsd_range / 500,
        "bins": initialize_bins(jsd_range / 500, jsd_range),
    }
    ens_diff_bins = {
        "size": ens_diff_range / 500,
        "bins": initialize_bins(ens_diff_range / 500, ens_diff_range),
    }
    ...
    while iteration_count < max_iterations:
        ...
        if outgroup_species:
            ...
            parent_node = ens_tree.lowest_common_ancestor(
                ingroup_species_pair
            ).parent.get_node_names()[0]
            lca_node = ens_tree.lowest_common_ancestor(
                ingroup_species_pair
            ).get_node_names()[0]
            try:
                triat_data, triple_alignment = get_triples_info(
                    triples_species_names,
                    ens_tree,
                    sequence,
                    result_lf,
                ...
                ens_diff_bins, processed_ens_diff = triples_binning_ens_diff(
                    triat_data, ens_diff_bins
                )
...
@click.option(
    ...
    required=True,
    help="Path to the directory containing sequence files.",
)
@click.option(
    "-a",
    "--alignment_dir",
    type=click.Path(exists=True),
    required=True,
    help="Path to the directory containing alignment files.",
)
@click.option(
    "-n",
    "--num_processes",
...
@click.option(
    ...
    "--output_dir",
    type=click.Path(),
    help="Path to the out directory for taxanomi triples",
)
@click.option(
    "-oa",
    "--output_alignment_dir",
    type=click.Path(),
    help="Path to the out directory for taxanomi triples alignments",
)
...
def main(
    ...
    result_input_dstore = open_data_store(
        input_path, suffix="json", mode="r", limit=limit
    )
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        pool.starmap(
            process_path,
            [
                (path, output_dir, sequence_dir, alignment_dir, output_alignment_dir)
                for path in result_input_dstore.completed
            ],
        )
