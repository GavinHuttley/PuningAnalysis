...
def process_model_fitting_results(gene_path, bounary_violation_function):
    ...
    model_fitting_results_input_dstore = open_data_store(
        model_fitting_result_dir, suffix="json"
    )
    ...
    for path in model_fitting_results_input_dstore:
        ...
        if not isinstance(model_fitting_result, NotCompleted):
            ...
            bounary_violation_check = bounary_violation_function.main(
...
def save_valid_identifiers(valid_triads_identifier_dict):
    ...
    with open(
        "/Users/gulugulu/clock/mammal_orthologs_hsap_1/valid_triads_identifier.json",
        "w",
    ) as outfile:
...
def process_gene_info(base_dir, valid_triads_identifier_dict):
    ...
    for gene_name, valid_triads_identifier in valid_triads_identifier_dict.items():
        triples_info_dir = os.path.join(
            base_dir, gene_name, "triples_info_dict_new.json"
        )
        ...
        for identifier in valid_triads_identifier:
            triples_species_name = triples_info_dict[identifier][
                "triples_species_names"
            ]
            ens_dict = triples_info_dict[identifier]["triples_info_small_tree"]["ens"]
            matrices = triples_info_dict[identifier]["triples_info_small_tree"][
                "matrices"
            ]

            matrices_full = {
                "ingroup1": np.array(matrices[triples_species_name["ingroup1"]])
                * ens_dict[triples_species_name["ingroup1"]],
                "ingroup2": np.array(matrices[triples_species_name["ingroup2"]])
                * ens_dict[triples_species_name["ingroup2"]],
                "outgroup": np.array(matrices[triples_species_name["outgroup"]])
                * ens_dict[triples_species_name["outgroup"]],
            }
            matrices_full_list.append(matrices_full)

            ens_pairs.append(
                {
                    triples_species_name["ingroup1"]: ens_dict[
                        triples_species_name["ingroup1"]
                    ],
                    triples_species_name["ingroup2"]: ens_dict[
...
def save_matrices_dict_full(matrices_dict_full):
    ...
    with open(
        "/Users/gulugulu/clock/mammal_orthologs_hsap_1/valid_matrix_full.json", "w"
    ) as outfile:
...
def main():
    ...
    for path in gene_paths:
        ...
        valid_triads_identifier = process_model_fitting_results(
            path, bounary_violation_function
        )
